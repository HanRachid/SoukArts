...
const activeLinkStyle = (linkName:string) => currentPage.split("/")[0] === linkName.toLowerCase() ? "bg-gradient-to-r from-colorBrown to-colorGold text-white" : "text-black"  

return (
    <section className="w-full h-full bg-gray-50 overflow-hidden">
      {showSidebar && (
        <div
          onClick={toggleSidebar}
          className="w-screen h-full fixed bg-black/50 backdrop-blur-sm z-10"
        />
      )}
      <aside
            ...
            {links.map(({ icon, linkName }) => (
              <li key={linkName}>
                <Link
                  ...
                  className={`${activeLinkStyle(linkName)} relative flex items-center space-x-4 rounded-xl px-4 py-3`}
                >
                  {icon}
